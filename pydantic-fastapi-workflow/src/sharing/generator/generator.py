...
class Generator:
    ...
    def process_query(self, filestem: str, query: str) -> str:
        ...
        # Assuming process_data.args and process_data.optional_args are populated as shown in your snippet
        ...
        print(process_data.optional_args)

        input_model_fields = []
        for arg_name, arg_info in {
            **process_data.args,
            **process_data.optional_args,
        }.items():
            field_type = arg_info.type_str
            optional = arg_info.optional
            default_value = "None" if optional else "..."
            input_model_fields.append((arg_name, field_type, default_value))

        # Store the input model representation in process_data or directly pass it to the template
        process_data.input_model = input_model_fields
        process_data.input_model_name = f"{snake_to_camel(filestem)}Input"
        print("input_model")
        print(process_data.input_model)
        print("input model end")
        ...
        rendered = template.render(
            ...
            input_model=process_data.input_model,
            input_model_name=process_data.input_model_name,
        )
